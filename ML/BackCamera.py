...
video_location: str = "dataset/B.mp4"
...
cv.namedWindow("Back Camera", WINDOW_NORMAL)
cv.resizeWindow("Back Camera", width, height)
                            ...
                            if box.id:
                                ...
                                if detected_class == 2:
                                    ...
                                    if not self.detected[detected_id]["extracted"] and (count_line_position + count_line_offset) > y > (count_line_position - count_line_offset):
                                        ...
                                        # Close Barrier in Thread
                                        thread: Thread = Thread(target = close_barrier)
                                        thread.start()
...
if __name__ == "__main__":
    back_camera: BackCamera = BackCamera()
    back_camera.start()
